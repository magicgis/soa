 ...
 */
package com.yeepay.g3.app.soa.center.monitor;
...
public class RegistryContainer implements Container {
	...
	private final Map<String, Set<String>> providerFacadeApplications = new ConcurrentHashMap<String, Set<String>>();

	private final Map<String, Set<String>> providerApplicationServices = new ConcurrentHashMap<String, Set<String>>();

	private final Map<String, Set<String>> consumerFacadeApplications = new ConcurrentHashMap<String, Set<String>>();
		...
		if (reverse) {
			...
			if (services != null && services.size() > 0) {
				for (String service : services) {
					Set<String> applications = consumerFacadeApplications.get(service);
			...
			if (services != null && services.size() > 0) {
				for (String service : services) {
					Set<String> applications = providerFacadeApplications.get(service);
					if (applications != null && applications.size() > 0) {
						dependencies.addAll(applications);
					}
				}
				...
				for (URL url : urls) {
					...
					if (Constants.PROVIDERS_CATEGORY.equals(category)) {
						if (Constants.EMPTY_PROTOCOL.equals(url.getProtocol())) {
							...
							list.add(url);
							if (application != null && application.length() > 0) {
								Set<String> serviceApplications = providerFacadeApplications.get(service);
								if (serviceApplications == null) {
									providerFacadeApplications.put(service, new ConcurrentHashSet<String>());
									serviceApplications = providerFacadeApplications.get(service);
								}
						...
						if (Constants.EMPTY_PROTOCOL.equals(url.getProtocol())) {
							...
							list.add(url);
							if (application != null && application.length() > 0) {
								Set<String> serviceApplications = consumerFacadeApplications.get(service);
								if (serviceApplications == null) {
									consumerFacadeApplications.put(service, new ConcurrentHashSet<String>());
									serviceApplications = consumerFacadeApplications.get(service);
								}
								serviceApplications.add(application);
