...
public class ConsumerServiceImpl extends AbstractService implements ConsumerService {
	...
	public Set<String> findAddressesByService(String service) {
		Set<String> ret = Sets.newHashSet();
		ConcurrentMap<String, Map<Long, URL>> consumerUrls = getRegistryCache().get(Constants.CONSUMERS_CATEGORY);
		if (null == consumerUrls || consumerUrls.get(service) == null) return ret;
	...
	public Set<String> findApplicationsByServiceName(String service) {
		Set<String> ret = Sets.newHashSet();
		ConcurrentMap<String, Map<Long, URL>> consumerUrls = getRegistryCache().get(Constants.CONSUMERS_CATEGORY);
		if (consumerUrls == null) return ret;
	...
	public void updateConsumer(Consumer consumer) {
		...
		Consumer oldConsumer = findConsumer(id);
		if (oldConsumer == null) {
			throw new IllegalStateException("Consumer was changed!");
		}
		URL newConsumer = consumer.toUrl();

		registryService.unregister(oldConsumer.toUrl());
		registryService.register(newConsumer);
	}
