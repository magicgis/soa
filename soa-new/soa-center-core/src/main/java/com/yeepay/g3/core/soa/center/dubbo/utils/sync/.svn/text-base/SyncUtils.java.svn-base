...
public class SyncUtils {
	...
	public static com.yeepay.g3.facade.soa.center.dubbo.domain.Override url2Override(Pair<Long, URL> pair) {
		if (pair == null) {
			return null;
		}
		...
		com.yeepay.g3.facade.soa.center.dubbo.domain.Override o = new com.yeepay.g3.facade.soa.center.dubbo.domain.Override();
		o.setId(id);
	...
	public static List<com.yeepay.g3.facade.soa.center.dubbo.domain.Override> url2OverrideList(Map<Long, URL> cs) {
		List<com.yeepay.g3.facade.soa.center.dubbo.domain.Override> list = new ArrayList<com.yeepay.g3.facade.soa.center.dubbo.domain.Override>();
		if (cs == null) return list;
		for (Map.Entry<Long, URL> entry : cs.entrySet()) {
		...
		for (Map.Entry<Long, URL> entry : from.entrySet()) {
			...
			for (Map.Entry<String, String> e : filter.entrySet()) {
				...
				if (ADDRESS_FILTER_KEY.equals(key)) {
					boolean withPort = value.indexOf(':') > 0;
					if ((withPort && !value.equals(url.getAddress()))
							|| (!withPort && !value.equals(url.getHost()))) {
						match = false;
						break;
					}
				} else {
